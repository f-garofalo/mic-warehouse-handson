...
func (uc *CreateArticleUseCase) Execute(ctx context.Context, in CreateArticleInput) (*CreateArticleOutput, error) {
	if strings.TrimSpace(in.ID) == "" {
		return nil, errors.New("CreateArticle: ID is required")
	}
	...
	// Build the canonical creation event here, but dispatch it only after Save
	// succeeds. Phase 05 intentionally keeps transport concerns out of the
	// aggregate: the aggregate owns invariants, while the use case owns workflow
	// ordering.
